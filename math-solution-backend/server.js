  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: `Process this math problem and provide a structured solution in form of json collection where each document represents a step in the solution and the fields should be step, then description and then an equation representing the description : ${input}` }
      ],
    });
...
app.post('/api/mathpix', async (req, res) => {
  ...
  try {
    ...
    console.error('MathPix Error:', error.response ? error.response.data : error.message);
